-- Sophie Ontology Model for Singapore Research
-- 6 tables: industries, topics, context_facts, audience_presets, probe_templates, survey_patterns

-- ============================================================
-- 1. INDUSTRIES
-- ============================================================
CREATE TABLE IF NOT EXISTS sophie_industries (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
...
-- ============================================================
-- 2. TOPICS (industry × scenario_type)
-- ============================================================
CREATE TABLE IF NOT EXISTS sophie_topics (
...
CREATE INDEX IF NOT EXISTS idx_topics_industry_scenario ON sophie_topics(industry_id, scenario_type);

-- ============================================================
-- 3. CONTEXT FACTS (per topic — Singapore-specific knowledge)
-- ============================================================
CREATE TABLE IF NOT EXISTS sophie_context_facts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_facts_topic ON sophie_context_facts(topic_id);

-- ============================================================
-- 4. AUDIENCE PRESETS (per topic)
-- ============================================================
CREATE TABLE IF NOT EXISTS sophie_audience_presets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  topic_id UUID NOT NULL REFERENCES sophie_topics(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_audience_topic ON sophie_audience_presets(topic_id);

-- ============================================================
-- 5. PROBE TEMPLATES (per industry × scenario — guides Sophie's questions)
-- ============================================================
CREATE TABLE IF NOT EXISTS sophie_probe_templates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  industry_id TEXT NOT NULL REFERENCES sophie_industries(id) ON DELETE CASCADE,
  scenario_type TEXT NOT NULL CHECK (scenario_type IN ('policy_simulation', 'product_pricing', 'audience_intelligence')),
...
CREATE INDEX IF NOT EXISTS idx_probes_industry_scenario ON sophie_probe_templates(industry_id, scenario_type);

-- ============================================================
-- 6. SURVEY PATTERNS (universal — question design templates)
-- ============================================================
CREATE TABLE IF NOT EXISTS sophie_survey_patterns (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  name_zh TEXT NOT NULL,
...
-- ============================================================
-- DISABLE RLS for these tables (public read, admin write)
-- ============================================================
...
CREATE POLICY "Allow public read" ON sophie_industries FOR SELECT USING (true);
CREATE POLICY "Allow public read" ON sophie_topics FOR SELECT USING (true);
CREATE POLICY "Allow public read" ON sophie_context_facts FOR SELECT USING (true);
CREATE POLICY "Allow public read" ON sophie_audience_presets FOR SELECT USING (true);
CREATE POLICY "Allow public read" ON sophie_probe_templates FOR SELECT USING (true);
CREATE POLICY "Allow public read" ON sophie_survey_patterns FOR SELECT USING (true);
...
-- Government × Audience Intelligence
INSERT INTO sophie_topics (id, industry_id, scenario_type, name, name_zh, description, description_zh, keywords, sort_order) VALUES
...
-- Real Estate × Audience Intelligence
INSERT INTO sophie_topics (id, industry_id, scenario_type, name, name_zh, description, description_zh, keywords, sort_order) VALUES
...
-- ═══════════════════════════════════════════════════
-- CONTEXT FACTS (Singapore-specific knowledge per topic)
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- PROBE TEMPLATES (Sophie's guided questions by industry × scenario × stage)
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- SURVEY PATTERNS (universal question design templates)
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- AUDIENCE PRESETS (selected topics)
-- ═══════════════════════════════════════════════════

-- CPF: all working-age adults
...
-- Eldercare policy: older adults and caregivers
INSERT INTO sophie_audience_presets (topic_id, age_min, age_max, rationale, rationale_zh) VALUES
...
-- New product concept: working-age consumers
INSERT INTO sophie_audience_presets (topic_id, age_min, age_max, rationale, rationale_zh) VALUES
...
-- HDB policy: young couples and families
INSERT INTO sophie_audience_presets (topic_id, age_min, age_max, rationale, rationale_zh) VALUES
...
-- Property cooling: property owners and potential buyers
INSERT INTO sophie_audience_presets (topic_id, age_min, age_max, rationale, rationale_zh) VALUES
